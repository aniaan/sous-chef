...
When bumping the plugin version, update the `version` field in `metadata.lua`. This is the source of truth for the plugin version exposed to `mise`/`vfox`, and it must be updated as part of any version release.
